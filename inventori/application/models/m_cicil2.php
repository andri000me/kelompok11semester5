...
class M_cicil2 extends CI_Model
{
    ...
    public function total_rows()
    {
        if ($this->session->userdata('level') == 'admin' or $this->session->userdata('level') == 'manajer') {
            ...
            $where = "(detail_keluar.status='0' and keluar.status='2' and keluar.del='0' and cicil.status='1' and cicil.del='0') or (detail_keluar.status='1'  and keluar.status='2' and keluar.del='0' and cicil.status='1' and cicil.del='0')";
            $this->db->where($where);
            ...
            $this->db->join('user', 'user.id_user=keluar.id_user');
            $this->db->join('cicil', 'cicil.id_keluar=keluar.id_keluar');
            $where = "(detail_keluar.status='0'  and keluar.status='2' and keluar.del='0' and cicil.status='1' and cicil.del='0' and user.id_user=$id) or (detail_keluar.status='1'  and keluar.status='2' and keluar.del='0' and cicil.status='1' and cicil.del='0' and user.id_user=$id)";
            $this->db->where($where);
        }
